...
function PropertyCard({ property, featured }: { property: (typeof Route.useLoaderData extends () => infer T ? T : never)[number]; featured: boolean }) {
  return <Link to="/properties/$slug" params={{ slug: property.slug }} className={`property-card ${featured ? "property-card-featured" : ""}`}>
    <div className="property-image"><img src={homeImage} width={1600} height={1008} loading="lazy" alt={`${property.title} exterior`} /><span className="status-tag">{property.status === "under_offer" ? "Under offer" : property.status}</span><span className="card-arrow"><MoveUpRight size={17} /></span></div>
